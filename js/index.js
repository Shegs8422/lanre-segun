...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelector(".loader").style.animation =
    "slideDown 1s ease-in-out forwards";

  // Add a delay before hiding the loader to ensure the animation completes
  setTimeout(() => {
    document.querySelector(".loader").style.display = "none";
  }, 500); // Adjust the timeout value to match the animation duration (1s in this case)
  document.querySelector(".loader").style.borderRadius = "0";
  document.querySelector(".loader").style.border = "100%";
  if (!/Mobi|Android/i.test(navigator.userAgent)) {
...
document.querySelector(".badges").addEventListener("click", function () {
  window.open("https://github.com/Shegs8422", "_blank");
});

document.querySelector(".youtube").addEventListener("click", function () {
  window.open(
    "https://artsandculture.google.com/entity/leonardo-da-vinci/m04lg6?hl=en",
    "_blank"
  );
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const socialLinks = {
    facebook: "https://www.facebook.com/oluwasegun.olanrewaju.2020/",
    x: "https://x.com/Olusegun51",
    linkedin: "https://www.linkedin.com/in/oluwasegun-olanrewaju-b847bb188/",
    fiverr: "https://www.fiverr.com/s/NNryXDG",
    discord: "https://discordapp.com/users/913339218330259456",
    dribbble: "https://dribbble.com/lanre_segun",
  };

  // Add click listeners to each award image
  awardsContainer.querySelectorAll(".award img").forEach((img) => {
    img.addEventListener("click", function () {
      const social = this.className;
    ...
    .then((projectData) => {
      ...
      document.querySelectorAll(".project").forEach((projectElement) => {
        ...
        projectElement.addEventListener("click", () => {
          ...
          document.querySelector(".details-of-project > h1").textContent =
            currentProject.name;
          document.querySelector(".details-of-project > p").textContent =
            currentProject.description;
          if (currentProject.demotype === "video") {
            document.querySelector(".project-infos > img").style.display =
              "none";
            document.querySelector(".project-infos > video").style.display =
              "block";
            document.querySelector(".project-infos > video").src =
              currentProject.demo;
          } else {
            document.querySelector(".project-infos > video").style.display =
              "none";
            document.querySelector(".project-infos > img").style.display =
              "block";
            document.querySelector(".project-infos > img").src =
              currentProject.demo;
          }
          if (currentProject.link) {
            document.querySelector(".project-under > a").style.display =
              "block";
            document.querySelector(".project-under > a").href =
              currentProject.link;
          } else {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateCardPositions() {
    ...
    pathBoxes.forEach((box) => {
      ...
      const left =
        position.x + (svg.getBoundingClientRect().left - containerRect.left);
      const top =
        position.y + (svg.getBoundingClientRect().top - containerRect.top);
